...
import ipca.app.buildingbug.databinding.ActivityProcurarServicoBinding
...
class ProcurarServicoActivity : AppCompatActivity() {

    private lateinit var binding: ActivityProcurarServicoBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityProcurarServicoBinding.inflate(layoutInflater)
        setContentView(binding.root)

        servicoDisponiveisAdapter = ProcurarServicoAdapter(this)
        binding.procurarServicoListView.adapter = servicoDisponiveisAdapter // corrected the ID to use binding
        val sharedPreferences = getSharedPreferences("MyPrefs", Context.MODE_PRIVATE)
        val token = sharedPreferences.getString("Token", null) // Variable names should start with a lowercase letter
        if (token != null) {
            getServicosDisponiveis(token)
        } else {
    ...
    private fun getServicosDisponiveis(token: String) {
        CoroutineScope(Dispatchers.IO).launch {
            var response: Response? = null
            try {
                val client = OkHttpClient()
                val request = Request.Builder()
                    .url("https://buildingbugapiapi.azure-api.net/api/v$apiVersion/Servico/all/disponivel")
                    .get()
                ...
                withContext(Dispatchers.Main) {
                    if (response.isSuccessful && responseBodyString != null) {
                        ...
                        Toast.makeText(this@ProcurarServicoActivity, "Failed to get services: $errorBody", Toast.LENGTH_SHORT).show()
                    }
                }
            } catch (e: IOException) {
                withContext(Dispatchers.Main) {
                    Toast.makeText(this@ProcurarServicoActivity, "Network error: ${e.message}", Toast.LENGTH_SHORT).show()
                }
            } catch (e: JsonParseException) {
                withContext(Dispatchers.Main) {
                    Toast.makeText(this@ProcurarServicoActivity, "JSON parsing error: ${e.message}", Toast.LENGTH_SHORT).show()
                }
            } catch (e: Exception) {
                withContext(Dispatchers.Main) {
                    Toast.makeText(this@ProcurarServicoActivity, "Unknown error: ${e.message}", Toast.LENGTH_SHORT).show()
                }
            } finally {
                response?.body?.close()
            }
    ...
    inner class ProcurarServicoAdapter(private val context: Context) : BaseAdapter() {
        ...
        override fun getView(position: Int, convertView: View?, parent: ViewGroup?): View {
            ...
            buttonIr.setOnClickListener {
                val intent = Intent(context, AceitarServicoDetailActivity::class.java)
                ...
                intent.putExtra("estadoServicoId", servico.estadoServico)
